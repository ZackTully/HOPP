...
class HeuristicLoadFollowingDispatchZCT(SimpleBatteryDispatchHeuristicZCT):
    ...
    def set_fixed_dispatch(self, gen: list, grid_limit: list, goal_power: list, start_time=None):
        ...
        self.check_gen_grid_limit(gen, grid_limit) # check that generation and grid limit signal are the same length
        self._set_power_fraction_limits(gen, grid_limit) # Max charge fraction from generation, max discharge from how much is left (grid - generation)
        # self._heuristic_method(gen, goal_power)
        self._heuristic_method_zct(gen, goal_power)
        self._fix_dispatch_model_variables() # fix the schedule in the pyomo model
    ...
    def _heuristic_method_zct(self, gen, goal_power):

            
        K_soc = 0.1

        mean_goal_power = np.mean(gen)

        for t in self.blocks.index_set():
            # fd = (mean_goal_power - gen[t]) / self.maximum_power
            fd = (goal_power[t] - gen[t]) / self.maximum_power # + K_soc * (55 - self.soc[0]) / 100
            if fd > 0.0:  # Discharging
                if fd > self.max_discharge_fraction[t]:
                    fd = self.max_discharge_fraction[t]
            elif fd < 0.0:  # Charging
