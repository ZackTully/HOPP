...
def batch_generator_kernel(arg_list):
    ...
    from hybrid.sites import (
        flatirons_site as sample_site,
    )  # For some reason we have to pull this inside the definition
    ...
    * Pipeline Model
    ...
    if electrolyzer_degradation_penalty == True:
        user_defined_stack_replacement_time = (
            False  # if true then not dependent on pem performance and set to constant
        )
    else:
    ...
    st_xl = pd.read_csv(
        parent_path + "/examples/H2_Analysis/storage_costs_ATB.csv", index_col=0
    )
    ...
    if run_RODeO_selector == True:
        ...
        hydrogen_lifecycle_emissions = LCA_single_scenario.hydrogen_LCA_singlescenario(
            grid_connection_scenario,
            atb_year,
            site_name,
            turbine_model,
        ...
        max_hydrogen_delivery_rate_kg_hr = np.max(
            hydrogen_production_storage_system_output_kgprhr
        )
    ...
    lime_unit_cost = (
        site_df["Lime ($/metric tonne)"] + site_df["Lime Transport ($/metric tonne)"]
    )
    carbon_unit_cost = (
    ...
    if run_RODeO_selector == True:
        ...
        print(
            "ammonia price: ",
            ammonia_price_breakdown["Ammonia price: Total ($/kg)"],
            " steel price: ",
            steel_price_breakdown["Steel price: Total ($/tonne)"],
        )
