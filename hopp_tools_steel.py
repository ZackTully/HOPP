...
YamlIncludeConstructor.add_to_loader_class(
    loader_class=yaml.FullLoader, base_dir=PATH / "floris_input_files/"
)
...
class hoppDict:
    ...
    def add(self, sub_dict_name, sub_dict) -> None:
        if sub_dict_name in self.main_dict.keys():
            self.main_dict[sub_dict_name] = self.update(
...
def set_electrolyzer_info(
    ...
    if atb_year == 2020:
        ...
        if electrolyzer_cost_case == "Low" or electrolyzer_cost_case == "low":
            component_costs_centralized = {
                "Stack": 807.3,
                "Power Electronics": 129.5,
                "BOP": 82.8,
                "H2 Conditioning": 94.8,
            }
        else:
            component_costs_centralized = {
                "Stack": 807.3,
        ...
        if electrolyzer_cost_case == "Low" or electrolyzer_cost_case == "low":
            component_costs_centralized = {
                "Stack": 227.9,
                "Power Electronics": 89.7,
                "BOP": 55.5,
                "H2 Conditioning": 77.0,
            }
        else:
            component_costs_centralized = {
                "Stack": 524.6,
                "Power Electronics": 127.5,
                "BOP": 81.4,
                "H2 Conditioning": 94.0,
            }
        component_scales_centralized = {
            "Stack": 10000,
            "Power Electronics": 40000,
        ...
        if electrolyzer_cost_case == "Low" or electrolyzer_cost_case == "low":
            component_costs_centralized = {
                "Stack": 90.0,
                "Power Electronics": 39.3,
                "BOP": 22.6,
                "H2 Conditioning": 48.1,
            }
        else:
            component_costs_centralized = {
                "Stack": 182.8,
                "Power Electronics": 74.5,
                "BOP": 45.4,
                "H2 Conditioning": 69.3,
            }
        component_scales_centralized = {
            "Stack": 10000,
            "Power Electronics": 40000,
        ...
        if electrolyzer_cost_case == "Low" or electrolyzer_cost_case == "low":
            component_costs_centralized = {
                "Stack": 64.5,
                "Power Electronics": 28.9,
                "BOP": 16.2,
                "H2 Conditioning": 40.5,
            }
        else:
            component_costs_centralized = {
                "Stack": 147.7,
                "Power Electronics": 61.6,
                "BOP": 36.9,
                "H2 Conditioning": 62.2,
            }
        component_scales_centralized = {
            "Stack": 10000,
            "Power Electronics": 40000,
    ...
    if electrolysis_scale == "Distributed":
        component_costs_distributed = {
            ...
            * (
                component_scales_centralized["Stack"]
                / component_scales_distributed["Stack"]
            )
            * (
                component_scales_distributed["Stack"]
                / component_scales_centralized["Stack"]
            ...
            * (
                component_scales_centralized["BOP"]
                / component_scales_distributed["BOP"]
            )
            * (
                component_scales_distributed["BOP"]
                / component_scales_centralized["BOP"]
        ...
        if direct_coupling:
            power_electronics_savings = (
                component_costs_distributed["Power Electronics"] * 0.3
            )
        else:
    ...
    if electrolysis_scale == "Distributed":
        capex_ratio_dist = electrolyzer_capex_kw / sum(
            component_costs_centralized.values()
        )
    elif electrolysis_scale == "Centralized":
        capex_ratio_dist = 1
...
def set_turbine_model(
    ...
    if floris == True:
        ...
        if grid_connection_scenario == "off-grid":
            turbine_file = os.path.join(
                floris_dir, "floris_input" + "_" + site_number + "_offgrid.yaml"
            )
        else:
            turbine_file = os.path.join(
                floris_dir, "floris_input" + "_" + site_number + "_gridconnected.yaml"
            )
    ...
    return (
        ...
        nTurbs,
        floris_config,
    )  # custom_powercurve_path, tower_height, rotor_diameter, turbine_rating_mw, wind_cost_kw, floating_cost_reductions_df, fixed_cost_reductions_df


def set_export_financials(
    ...
    export_system_installation_cost_kw = (
        site_df["Export System Installation"]
        + site_df["Offshore Substation Installation"]
    )
    total_export_system_cost_kw = (
        export_system_cost_kw + export_system_installation_cost_kw
    )
    ...
    export_system_installation_cost = (
        export_system_installation_cost_kw * wind_size_mw * 1000
    )
...
def set_turbine_financials(
    ...
    if turbine_model == "12MW":
        ...
        floating_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/floating_cost_reductions_12MW.csv"
            )
        )
        fixed_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/fixed_cost_reductions_12MW.csv"
            )
        )
        ...
        floating_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/floating_cost_reductions_15MW.csv"
            )
        )
        fixed_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/fixed_cost_reductions_15MW.csv"
            )
        )
        ...
        floating_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/floating_cost_reductions_18MW.csv"
            )
        )
        fixed_cost_reductions_df = pd.read_csv(
            os.path.join(
                parent_path, "examples/H2_Analysis/fixed_cost_reductions_18MW.csv"
            )
        )
    ...
    if print_wind_info:
        print(
            "Wind Cost in baseline year was {}, reduced to {} in {}".format(
                wind_cost_kw, new_wind_cost_kw, atb_year
            )
        )
        print(
            "Operation and Maintain Cost, reduced from {} to {}".format(
                wind_om_cost_kw, new_wind_om_cost_kw
            )
        )
        print(
            "Net Capacity Factor increased from {} to {}".format(
...
def print_h2_results2(
    ...
    if print_it:
        ...
        print(
            "For a {}MW Offshore Wind Plant with {}MW electrolyzer located at {} \n (average wind speed {}m/s) in {}, with a Wind CAPEX cost of {},\n and an Electrolyzer cost of {}$/kW:\n The levelized cost of hydrogen was {} /kg ".format(
                forced_wind_size,
                electrolyzer_size,
                site_name,
                np.average(wind_speed),
                atb_year,
                site_df["Total CapEx"],
                electrolyzer_capex_kw,
                LCOH_cf_method_w_operating_costs,
...
def run_battery(
    ...
    bat_model.charge_rate = (
        hopp_dict.main_dict["Configuration"]["storage_size_mw"] * 1000
    )
    bat_model.discharge_rate = (
        hopp_dict.main_dict["Configuration"]["storage_size_mw"] * 1000
    )
    bat_model.battery_storage = (
        hopp_dict.main_dict["Configuration"]["storage_size_mwh"] * 1000
    )
...
def pipeline_vs_hvdc(site_df, wind_size_mw, total_h2export_system_cost):
    ...
    total_export_system_cost = export_system_cost + export_system_installation_cost
    print(
        "Total HVDC Export System Cost is ${0:,.0f} vs ${1:,.0f} for H2 Pipeline".format(
            total_export_system_cost, total_h2export_system_cost
        )
    )
...
def run_H2_PEM_sim(
    ...
    if grid_connection_scenario != "off-grid":
        ...
        if grid_connection_scenario == "grid-only":
            hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
                "energy_from_the_grid_old"
            ] = hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
                "energy_from_the_grid"
            ]
            hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
                "energy_from_the_grid"
            ] = energy_signal_to_electrolyzer
            hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
                "energy_to_electrolyzer_old"
            ] = hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
                "energy_to_electrolyzer"
            ]
            hopp_dict.main_dict["Models"]["grid"]["ouput_dict"][
            ...
            power_from_grid = energy_signal_to_electrolyzer[
...
def grid(
    ...
    energy_to_electrolyzer = np.zeros_like(
        combined_pv_wind_storage_power_production_hopp
    )
    ...
    if buy_price:
        ...
        for i in range(len(combined_pv_wind_storage_power_production_hopp)):
            if combined_pv_wind_storage_power_production_hopp[i] < kw_continuous:
                ...
                energy_total[i] = (
                    energy_from_the_grid[i]
                    + combined_pv_wind_storage_power_production_hopp[i]
                )
        ...
        energy_to_electrolyzer = [
            x if x < kw_continuous else kw_continuous
            for x in combined_pv_wind_storage_power_production_hopp
        ]
...
def calculate_financials(
    ...
    cf_df.to_csv(
        os.path.join(
            results_dir,
            "Annual Cashflows_{}_{}_{}_discount_{}_{}MW.csv".format(
                site_name, scenario_choice, atb_year, discount_rate, turbine_rating_mw
            ),
        )
    )
    ...
    discounted_h2_production = npf.npv(
        discount_rate, [H2_Results["hydrogen_annual_output"]] * 30
    )
    ...
    LCOH_cf_method_total_pipeline = (
        -tlcc_total_costs_pipeline / discounted_h2_production
    )
    ...
    financial_summary_df.to_csv(
        os.path.join(
            results_dir,
            "Financial Summary_{}_{}_{}_{}.csv".format(
                site_name, atb_year, turbine_model, policy_option
            ),
        )
    )
    ...
    total_installed_and_operational_lifetime_cost_pipeline = (
        total_system_installed_cost_pipeline
        + (30 * total_annual_operating_costs_pipeline)
    )
    ...
    total_installed_and_operational_lifetime_cost_hvdc = (
        total_system_installed_cost_hvdc + (30 * total_annual_operating_costs_hvdc)
    )
    lifetime_h2_production = 30 * H2_Results["hydrogen_annual_output"]
...
def write_outputs_RODeO(
    ...
    steel_price_breakdown_df = pd.DataFrame.from_dict(
        steel_price_breakdown, orient="index"
    )
    ammonia_price_breakdown_df = pd.DataFrame.from_dict(
        ammonia_price_breakdown, orient="index"
    )
    ...
    financial_summary_df.to_csv(
        os.path.join(
            fin_sum_dir,
            "Financial_Summary_RODeO_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
...
def write_outputs_ProFAST(
    ...
    h2_transmission_capex = hopp_dict.main_dict["Models"][
        "levelized_cost_of_h2_transmission"
    ]["output_dict"]["H2 transmission capex"]
    steel_total_capex = hopp_dict.main_dict["Models"]["steel_LCOS"]["output_dict"][
        "steel_plant_capex"
    ]
    ammonia_total_capex = hopp_dict.main_dict["Models"]["levelized_cost_of_ammonia"][
        "output_dict"
    ]["ammonia_total_capex"]
    ...
    financial_summary_df = pd.DataFrame(
        [
            ...
            lcoh_breakdown["LCOH: Taxes ($/kg)"]
            + lcoh_breakdown["LCOH: Finances ($/kg)"],
    ...
    financial_summary_df.to_csv(
        os.path.join(
            fin_sum_dir,
            "Fin_sum_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
                electrolyzer_cost_case_string,
                policy_option,
                grid_string,
                renbat_string,
                windmodel_string,
    ...
    df_energy.drop(
        columns=["cost_to_buy_from_grid", "profit_from_selling_to_grid"], inplace=True
    )
    ...
    df_energy.to_csv(
        os.path.join(
            energy_profile_dir,
            "Energy_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
    ...
    profast_h2_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "H2_PF_PB_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
    ...
    profast_steel_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "Stl_PF_PB_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
    ...
    profast_ammonia_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "NH3_PF_PB_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.csv".format(
                site_name,
                atb_year,
                turbine_model,
                electrolysis_scale,
...
def write_outputs_ProFAST_SMR(
    ...
    financial_summary_df.to_csv(
        os.path.join(
            fin_sum_dir,
            "Financial_Summary_ProFAST_SMR_{}_{}_{}_{}_{}_heat_integration_{}.csv".format(
                site_name,
                atb_year,
                policy_case,
                CCS_option,
    ...
    profast_h2_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "H2_PF_PB_{}_{}_{}_{}_{}_heat_integration_{}.csv".format(
                site_name,
                atb_year,
                policy_case,
                CCS_option,
    ...
    profast_steel_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "Stl_PF_PB_{}_{}_{}_{}_{}_heat_integration_{}.csv".format(
                site_name,
                atb_year,
                policy_case,
                CCS_option,
    ...
    profast_ammonia_price_breakdown.to_csv(
        os.path.join(
            price_breakdown_dir,
            "NH3_PF_PB_{}_{}_{}_{}_{}_heat_integration_{}.csv".format(
                site_name,
                atb_year,
                policy_case,
                CCS_option,
...
def steel_LCOS(
    ...
    steel_capacity_factor = np.min(
        [0.9, hydrogen_annual_production / (8760 * max_hydrogen_delivery_kg_hr)]
    )
    steel_plant_life = 30
    ...
    max_steel_production_capacity_mtpy = (
        8760 * max_hydrogen_delivery_kg_hr / 1000 / hydrogen_consumption_for_steel
    )
    ...
    grid_prices = pd.read_csv(
        os.path.join(
            ...
            "examples/H2_Analysis/annual_average_retail_prices.csv",
        ),
        index_col=None,
        header=0,
    )
    elec_price = grid_prices.loc[grid_prices["Year"] == grid_year, site_name].tolist()[
        0
    ]
    # if site_name=='WY':
...
def levelized_cost_of_ammonia(
    ...
    if hopp_dict.save_model_input_yaml:
        ...
        hopp_dict.add(
            "Models", {"levelized_cost_of_ammonia": {"input_dict": input_dict}}
        )
    ...
    hydrogen_consumption_for_ammonia = (
        0.197284403  # kg of hydrogen/kg of ammonia production
    )
    # Could be good to make this more conservative, but it is probably fine if demand profile is flat

    # Could connect these to other things in the model
    # ammonia_capacity_factor = np.min([0.9, hydrogen_annual_production / (8760 * max_hydrogen_delivery_kg_hr)])
    ammonia_capacity_factor = hydrogen_annual_production / (
        8760 * max_hydrogen_delivery_kg_hr
    )
    ...
    max_ammonia_production_capacity_kgpy = (
        8760 * max_hydrogen_delivery_kg_hr / hydrogen_consumption_for_ammonia
    )
    ...
    if hopp_dict.save_model_output_yaml:
        ...
        hopp_dict.add(
            "Models", {"levelized_cost_of_ammonia": {"output_dict": output_dict}}
        )
...
def levelized_cost_of_h2_transmission(
    ...
    if hopp_dict.save_model_input_yaml:
        ...
        hopp_dict.add(
            "Models", {"levelized_cost_of_h2_transmission": {"input_dict": input_dict}}
        )
    ...
    elec_price = (
        grid_prices.loc[grid_prices["Year"] == grid_year, site_name].tolist()[0] / 1000
    )
    # if site_name=='WY':
    #     elec_price = grid_prices.loc[grid_prices['Year']==grid_year,'TX'].tolist()[0]
    # else:
    ...
    if policy_option == "no policy":
        ...
        if grid_connection_scenario == "grid-only":
            if electrolysis_total_EI_policy_grid <= 0.45:  # kg CO2e/kg H2
                ...
                electrolysis_total_EI_policy_grid > 1.5
                and electrolysis_total_EI_policy_grid <= 2.5
            ):  # kg CO2e/kg H2
                H2_PTC_grid = 0.75  # $/kg H2
            elif (
                electrolysis_total_EI_policy_grid > 2.5
                and electrolysis_total_EI_policy_grid <= 4
            ):  # kg CO2e/kg H2
...
def hydrogen_storage_capacity_cost_calcs(
    H2_Results, electrolyzer_size_mw, storage_type, dynamic_case
):
    H2_demand = calc_H2_demand(H2_Results["hydrogen_hourly_production"], dynamic_case)
    H2_storage_capacity_kg, H2_SOC = _calc_H2_storage(
        H2_Results["hydrogen_hourly_production"], H2_demand
    )

    h2_LHV = 119.96  # [GCV - gross calorific value]
    h2_HHV = 141.88  # [GCV - gross calorific value]
    hydrogen_storage_capacity_MWh_LHV = H2_storage_capacity_kg * h2_LHV / 3600
    hydrogen_storage_capacity_MWh_HHV = H2_storage_capacity_kg * h2_HHV / 3600
    ...
    )  # why is storage_capacit_LHV divided by electrolyzer_efficiency_HHV ???

    equation_year_CEPCI = 603.1
    model_year_CEPCI = 607.5

    storage_cost_USDprkg, status_message = _calc_H2_storage_cost(
...
def _calc_H2_demand_static(H2_hourly):
    H2_max = np.max(H2_hourly)
    H2_demand = np.ones(len(H2_hourly)) * np.mean(H2_hourly)
    # No industry process can operate below 10% of maximum
    industry_min = 0.1
    H2_demand = np.array([np.max([industry_min * H2_max, H2d]) for H2d in H2_demand])
    return H2_demand


def _calc_H2_demand_dynamic_ammonia(H2_hourly):
    ammonia_ramp_rate = 0.9  # 90% of rated per hour
    H2_max = np.max(H2_hourly)

    H2_demand = np.zeros(len(H2_hourly))
    for i in range(len(H2_hourly)):
        if i == 0:
            H2_demand[i] = H2_hourly[i]
        else:
            if (
                np.abs(H2_hourly[i] - H2_demand[i - 1]) > ammonia_ramp_rate * H2_max
            ):  # exceeding the ramp limit
                H2_demand[i] = (
                    H2_demand[i - 1]
                    + np.sign(H2_hourly[i] - H2_demand[i - 1])
                    * ammonia_ramp_rate
                    * H2_max
                )
            else:  # within the ramp limit
                H2_demand[i] = H2_hourly[i]
    ...
    for i in range(len(H2_demand)):
        H2_demand[i] = H2_demand[i] + H2_SOC_loop
        H2_demand[i] = np.max([H2_demand[i], industry_min * H2_max])
        H2_demand[i] = np.min([H2_demand[i], H2_max])
        H2_SOC_loop += H2_hourly[i] - H2_demand[i]

    return H2_demand


def _calc_H2_demand_dynamic_steel(H2_hourly):
    H2_max = np.max(H2_hourly)
    H2_demand = np.zeros_like(H2_hourly)
    for i in np.arange(0, 8760 - 71, 72):
        H2_demand[i : i + 72] = np.ones(72) * np.mean(H2_hourly[i : i + 72])
    # No industry process can operate below 10% of maximum
    industry_min = 0.1
    H2_demand = np.array([np.max([industry_min * H2_max, H2d]) for H2d in H2_demand])
    return H2_demand
...
def _calc_H2_storage(H2_gen, H2_demand):
    H2_suprlus_deficit = H2_gen - H2_demand
    H2_SOC = np.zeros(len(H2_gen))
    H2_SOC_loop = 0

    for i in range(len(H2_gen)):
        H2_SOC_loop += H2_suprlus_deficit[i]
        H2_SOC[i] = H2_SOC_loop

    H2_storage_capacity = np.max(H2_SOC) - np.min(H2_SOC)

    return H2_storage_capacity, H2_SOC


def _calc_H2_storage_cost(
    H2_storage_capacity_kg,
    hydrogen_storage_capacity_MWh_HHV,
    model_year_CEPCI,
    equation_year_CEPCI,
    ...
    if (
        ...
        if hydrogen_storage_capacity_MWh_HHV <= 120293:
            ...
            storage_cost_USDprkg = (
                ...
                * base_cost_USDprkg
                * (hydrogen_storage_capacity_MWh_HHV / base_capacity_MWh_HHV)
                ** scaling_factor
                / hydrogen_storage_capacity_MWh_HHV
            )
            status_message = (
                "Hydrogen storage model complete.\nStorage capacity: "
                + str(H2_storage_capacity_kg / 1000)
                + " metric tonnes. \nStorage cost: "
        ...
        if hydrogen_storage_capacity_MWh_HHV <= 4085:
            ...
            storage_cost_USDprkg = model_year_CEPCI / equation_year_CEPCI * 521.34
            status_message = (
                "Error: Please enter a valid hydrogen storage type. Otherwise, assuming buried pipe (location agnostic) hydrogen storage.\nStorage capacity: "
                + str(H2_storage_capacity_kg / 1000)
                + " metric tonnes. \nStorage cost: "
                + str(storage_cost_USDprkg)
                + " $/kg"
    ...
    return storage_cost_USDprkg, status_message
